...
- **최종 갱신**: 2026-09-23
- **상태**: 라이브 배포 완료 + 시간당 자동 갱신 정상 동작 (Lead time·PR 사이클 이상치 제외 적용)
...
- **이상치 제외(Lead time · PR 사이클)**: 비정상적으로 오래 걸린 극단 이상치(예: `AS-23469` 1,086h)가 주간 중앙값을 급등시키던 문제 방지. **Tukey 상단 울타리(Q3 + 1.5·IQR)를 넘는 개별 값만 중앙값 계산에서 제외**하고 모든 주는 그대로 표시(소표본 주를 통째로 숨기지 않음). `_drop_upper_outliers()` 헬퍼로 처리.
...
- **이슈↔커밋 연결 부족**: 커밋이 Jira 키를 잘 참조하지 않아 Lead time·PR 사이클의 AI/Human 비교 신뢰도가 낮음. → **극단 이상치 제외(Q3+1.5·IQR 초과)** 로 왜곡된 급등 표시는 막았으나, 근본 해결은 커밋의 Jira 키 참조 정착으로 연결 표본을 늘리는 것.
